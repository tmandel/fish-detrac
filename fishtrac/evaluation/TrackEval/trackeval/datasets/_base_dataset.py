...
class _BaseDataset(ABC):
    ...
    @staticmethod 
    def _calculate_box_diou_penalty_terms(tracker, gt):
        
        tracker_centroids = np.transpose([tracker[:, 0] + (tracker[:, 2] / 2), tracker[:, 1] + (tracker[:, 3] / 2)])

        gt_centroids = np.transpose([gt[:, 0] + (gt[:, 2] / 2), gt[:, 1] + (gt[:, 3] / 2)])
        
        centroid_distances = np.square(gt_centroids[np.newaxis, :, 0] - tracker_centroids[:, np.newaxis, 0]) \
                             + np.square(gt_centroids[np.newaxis, :, 1] - tracker_centroids[:, np.newaxis, 1])
        
        smallest_enclosing_boxes_x = np.square(np.maximum((gt[np.newaxis, :, 0] + gt[np.newaxis, :, 2]), tracker[:, np.newaxis, 0] + tracker[:, np.newaxis, 2]) - np.minimum(gt[np.newaxis, :, 0], tracker[:, np.newaxis, 0]))

        smallest_enclosing_boxes_y = np.square(np.maximum((gt[np.newaxis, :, 1] + gt[np.newaxis, :, 3]), tracker[:, np.newaxis, 1] + tracker[:, np.newaxis, 3]) - np.minimum(gt[np.newaxis, :, 1], tracker[:, np.newaxis, 1]))
          
        smallest_enclosing_boxes_diagonals = smallest_enclosing_boxes_x + smallest_enclosing_boxes_y
        
        diou_penalties = centroid_distances / smallest_enclosing_boxes_diagonals
       
        return diou_penalties

    @staticmethod
    def _calculate_box_dious(bboxes1, bboxes2, box_format='xywh'):
        ious = _BaseDataset._calculate_box_ious(bboxes1, bboxes2, box_format)
        diou_penalty_terms = _BaseDataset._calculate_box_diou_penalty_terms(bboxes1, bboxes2)
        dious = 1.0 - ious + diou_penalty_terms
...
if __name__ == "__main__":
    # TEST DIOU IMPLEMENTATION in main
    tracker = np.array([[10., 10., 10., 10.], [0., 0., 10., 10.], [20., 20., 10., 10.]])
    gt = np.array([[10., 10., 10., 10.], [0., 0., 20., 10.]])
    print("tracker:\n", tracker)
    print("gt:\n", gt)

    diou_penalties = _BaseDataset._calculate_box_diou_penalty_terms(tracker, gt)
