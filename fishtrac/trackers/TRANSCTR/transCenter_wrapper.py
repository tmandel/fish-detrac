...
#sys.path.append(os.path.abspath('./TransCenter'))
#import trackers.mot17_pub_test as transCenter
...
if len(sys.argv) < 6:
    print("Usage: transCentr_wrapper.py detectionsFile numFrames frameWidth frameHeight imgPath igrStr")
    sys.exit(2)

#TODO: Clear out the results from last time, if any
inputDir = './TransCenter-input/'
if os.path.exists(inputDir):
    os.system('rm -r ' + inputDir)
os.system('mkdir ' + inputDir)

outputDir = './TransCtr-output'
if os.path.exists(outputDir):
    os.system('rm -r ' + outputDir)
os.system('mkdir ' + outputDir)

testDir = './TransCenter-input/test/'
os.system('mkdir ' + testDir)

vidDir = './TransCenter-input/test/MOT17-video/'
os.system('mkdir ' + vidDir)

detDir = './TransCenter-input/test/MOT17-video/det/'
os.system('mkdir ' + detDir)

#gtDir = './TransCenter-input/test/MOT17-video/gt'
#os.system('mkdir ' + gtDir)
...
if "MVI" in imgPath: #car
    model = "./model_zoo/car_scratch.pth"
elif "MOT17" in imgPath: #ped
    model = "./model_zoo/car_scratch.pth"
else: #fish
    model = "./model_zoo/fish_scratch.pth"

os.system('rm ./TransCenter-input/test/MOT17-video/img1')
os.system('ln -s  ' + imgPath + ' ./TransCenter-input/test/MOT17-video/img1' )


d = scipy.io.loadmat(sys.argv[1])
detections = convertMatToBBList(d['detects'])

dataSetPath = os.path.abspath('./TransCenter-input')
detectFile = convert_input.convert_and_save_detections(detections, detDir)
print("about to run")
os.system("cd ./TransCenter_official/ && CUDA_VISIBLE_DEVICES=1 singularity exec --nv ../transcenter_singularity.sif python tracking/mot17_pub_test.py --data_dir="+dataSetPath+" --model_path=" + model)
#outFile = transCenter.main(imgPath, igrFile, detectFile, seqName, danOutputDir)
print("converting results")
convert_output.convert_DAN_results_to_detrac_input("MOT17-video", outputDir +"/txt", '.', numFrames)
print("done")
