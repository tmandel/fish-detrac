...
def _filter_correct(observation_matrix, observation_covariance,
        ...
        kalman_gain = (
            np.dot(predicted_state_covariance,
                   np.dot(observation_matrix.T,
                          linalg.pinvh(predicted_observation_covariance)))
        )
        #print("subtracting", observation, "and", predicted_observation_mean)
    ...
    # Removed kalman_gain return value
    return (None, corrected_state_mean,
            corrected_state_covariance)
...
def _filter(transition_matrices, observation_matrices, transition_covariance,
    ...
    for t in range(n_timesteps):
        ...
        #Edge case with Nones - need consistent format in filter
        if currentObs is not None and (currentObs[0] is None or np.ma.is_masked(currentObs)):
            currentObs = None
    ...
    kalman_smoothing_gain = (
        np.dot(filtered_state_covariance,
               np.dot(transition_matrix.T,
                      linalg.pinvh(predicted_state_covariance)))
    )
...
class KalmanFilter(object):
    ...
    def __init__(self, transition_matrices=None, observation_matrices=None,
        ...
        (self.transition_matrices, self.transition_offsets, self.transition_covariance,
    ...
    def filter_update(self, filtered_state_mean, filtered_state_covariance,
        ...
        predicted_state_mean, predicted_state_covariance = (
            _filter_predict(
                ...
                self.transition_offsets, filtered_state_mean,
                filtered_state_covariance
            )
        )
        (_, next_filtered_state_mean,
         next_filtered_state_covariance) = (
            _filter_correct(
                ...
                self.observation_offsets, predicted_state_mean,
                predicted_state_covariance, observation
            )
        )

        #if np.isnan(np.sum(next_filtered_state_mean)) or np.isnan(np.sum(next_filtered_state_covariance)):
        #    print("Error in filter_update!")
    ...
    def smooth(self, X):
        ...
        #if np.isnan(np.sum(smoothed_state_means)) or np.isnan(np.sum(smoothed_state_covariances)):
        #    print("Error in smooth!")
        return (smoothed_state_means, smoothed_state_covariances)
