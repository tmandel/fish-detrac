# Modified by Travis Mandel to suit the FISHTRAC codebase
import argparse
import itertools
import json
import logging
...
DET_COL_NAMES_DIRECT =('image_path', 'bb_left', 'bb_top', 'bb_width', 'bb_height', 'conf', 'category')
    ...
    def __getitem__(self, ix):
        ...
        if row['image_path'] != self.curr_image_path:
            ...
            if self.curr_img is None:
                ...
                print("Reading Image", row['image_path'], "returned", self.curr_img)
...
def getFrameIndexFromImageName(imageName):
    #../.././DETRAC-images/02_Oct_18_Vid-3/img00001.jpg --> 1
    return int(imageName[-9:-4])

def inferDirect(det_path, output_path, model_file):
    ...
    det_df = det_df[det_df.columns[:len(DET_COL_NAMES_DIRECT)]]
    det_df.columns = DET_COL_NAMES_DIRECT
    ...
    for i in range(len(det_df)):
         frameIndexes.append(getFrameIndexFromImageName(det_df['image_path'][i]))
    det_df['frame_index'] = frameIndexes

    data = np.hstack((det_df[['frame_index', 'category','bb_left', 'bb_top', 'bb_width', 'bb_height', 'conf']].values.astype(np.float32),features))
