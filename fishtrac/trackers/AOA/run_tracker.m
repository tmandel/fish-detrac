...
pythonPath = [options.condaPath  '/AOA_env/bin/python'];
%
%delete('results.mat')
...
try
    time_start = tic;
	method = [pythonPath ' aoa_tracker.py detections.mat ' num2str(numFrames) ' ' num2str(curSequence.imgWidth) ' ' num2str(curSequence.imgHeight) ' ' curSequence.imgFolder ' ' igrStr]
	disp(method)
	system(method);
	stateInfo = [];
	stateInfo.Xi = dlmread('AOA_LX.txt');
	stateInfo.Yi = dlmread('AOA_LY.txt');
	stateInfo.W = dlmread('AOA_W.txt');
	stateInfo.H =  dlmread('AOA_H.txt');
