...
def generateSingleTrackMedianFlow(startingIndex, numFrames, bbox, images, frameWidth, frameHeight):
	...
	newBox = None
	#instantiate tracker
	trackerStartBox = bbox#scaleBox(bbox, frameWidth, frameHeight, 'down')
	...
	while backwardIndex > 0:
		...
		if newBox is not None:
			...
			break  #Stop as soon as we hit the first None
	#look backward
...
def trackMF(detections, numFrames, frameWidth, frameHeight, images):
...
def track_medflow_matlab_wrapper(detections, numFrames, frameWidth, frameHeight, imgPath):
	...
	tracks = trackMF(detections, numFrames, frameWidth, frameHeight, images)
	end = time()
