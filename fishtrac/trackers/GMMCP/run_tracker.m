...
try
    time_start = tic;
	method = [pythonPath ' gmmcp_wrapper.py detections.mat ' num2str(numFrames) ' ' num2str(curSequence.imgWidth) ' ' num2str(curSequence.imgHeight) ' ' curSequence.imgFolder]
	disp(method)
	system(method);
	stateInfo = [];
	stateInfo.Xi = dlmread('GMMCP_LX.txt');
	stateInfo.Yi = dlmread('GMMCP_LY.txt');
	stateInfo.W = dlmread('GMMCP_W.txt');
	stateInfo.H =  dlmread('GMMCP_H.txt');
