...
try
    time_start = tic;
	method = [pythonPath ' jpda_wrapper.py detections.mat ' num2str(numFrames) ' ' num2str(curSequence.imgWidth) ' ' num2str(curSequence.imgHeight) ' ' curSequence.imgFolder]
	disp(method)
	system(method);
	load('stateInfo.mat'); %Creates stNew
	stateInfo = [];
	stateInfo.Xi = stNew.Xi;
	stateInfo.Yi = stNew.Yi;
	stateInfo.W = stNew.W;
	stateInfo.H =  stNew.H;
