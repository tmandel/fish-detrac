...
class Feat2StageAlg(object):
    ...
    def create_bb_graph(self, bbs, frames, X):
        ...
        new_vs = bb_graph.vs.select(list(range(old_vcount, old_vcount+bb_count)))
        new_vs["bbox"] = bbs
        new_vs["frame"] = frames
        if self.W_APP > 0.0:
            new_vs["feat"] = X
    ...
    def create_tracklet_graph(self, bb_graph, frame_count):
        ...
        # Create tracklets
        for i in range(self.MAX_TRACKLET_COUNTS[0]+1):
    ...
    def extract_tracks(self, t_graph):
        ...
        tracks = list()
        # Run dijkstra from bb_source to bb_sink
        for i in range(self.MAX_TRACKLET_COUNTS[1]+1):
