...
class HOG(object):
    def __init__(self, orientations=40, pixels_per_cell=(8, 8),
        cells_per_block=(1, 1), normalise=False):
    ...
    def __str__(self):
        str = "HOG:\n"
        str += "    orientations:    {}\n".format(self.orientations)
        str += "    pixels_per_cell: {}\n".format(self.pixels_per_cell)
        str += "    cells_per_block: {}\n".format(self.cells_per_block)
        str += "    normalise:       {}".format(self.normalise)
        return str

    def compute(self, img):
        image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        hog_features = hog(image, orientations=self.orientations,
