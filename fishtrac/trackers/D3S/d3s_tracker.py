...
env_path = os.path.join(os.path.dirname(__file__), 'd3s_repo')
if env_path not in sys.path:
	sys.path.append(env_path)

import pytracking
from d3s_repo import segm
import pytracking.parameter.segm.default_params as param_module
...
def getNewBoxColorTracker( newImage, frameIndex, tracker, frameWidth, frameHeight, direction):
	...
	if direction == 'forward':
		if tracker['highestFrameIndex'] == frameIndex:
			...
			located, newPoly = tracker['forwardTracker'].track(newImage)
			print("newBox is", newPoly)
			newBox = fromPolyForm(newPoly)
			if not located:
				newBox=None
				#print('Not Located')
	...
	#BACKWARD
		...
		if tracker['lowestFrameIndex'] == frameIndex:
			...
			located, newPoly = tracker['backwardTracker'].track(newImage)
			print("newBox is", newPoly)
			newBox = fromPolyForm(newPoly)
			if not located:
				newBox=None
...
def initExternalTracker( startFrameIndex, startImage, startBox):
	#Instantiate D3S tracker
	forwardTracker = segm.Segm(get_parameters())
	print("startbox is", startBox, "changed to", toPolyForm(startBox))
	possibleFor = forwardTracker.initialize(startImage, toPolyForm(startBox))
	#if not possibleFor:
		#print('Line 1498, cv.tracker.init failed')
	backwardTracker = segm.Segm(get_parameters())
	possibleBack=backwardTracker.initialize(startImage, toPolyForm(startBox))
	#if not possibleBack:
		#print('Line 1498, cv.tracker.init failed')
	#create dictionary
	visTracker = {}
...
def generateSingleTrackD3S(startingIndex, numFrames, bbox, images, frameWidth, frameHeight):
	...
	trackerD3S = initExternalTracker(startingIndex, images[startingIndex], trackerStartBox)
	backwardIndex = startingIndex
	forwardIndex = startingIndex+1
	#look forward
	while backwardIndex > 0:
		backwardIndex-=1
		newBox = getNewBoxColorTracker(images[backwardIndex], backwardIndex, trackerD3S, frameWidth, frameHeight, 'backward')
	...
	while forwardIndex < numFrames:
		newBox = getNewBoxColorTracker(images[forwardIndex], forwardIndex, trackerD3S, frameWidth, frameHeight, 'forward')
...
def trackD3S(detections, numFrames, frameWidth, frameHeight, images):
	...
	while maxConfidence > MIN_START_CONFIDENCE:# and len(allTracks)<11:
		...
		if maxConfidence > MIN_START_CONFIDENCE:
			...
			track = generateSingleTrackD3S(maxFrame-1, numFrames, maxBox, images, frameWidth, frameHeight)
			for (i,b) in track:
				#print('I,B***', i,b)
				#if(b[2]>(frameWidth/3)):
...
def track_kpd_matlab_wrapper(detections, numFrames, frameWidth, frameHeight, imgPath):
	...
	tracks = trackD3S(detections, numFrames, frameWidth, frameHeight, images)
