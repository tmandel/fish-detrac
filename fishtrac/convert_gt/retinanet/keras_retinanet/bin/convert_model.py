...
from retinanet.keras_retinanet.utils.config import read_config_file, parse_anchor_parameters
...
def ConvertAndSave(prediction_model, model, model_out, detector=None, args=[]):
    # parse arguments
    args = parse_args(args)
    # optionally load config parameters
    anchor_parameters = None
    ...
    # convert the model
    model, detector = models.convert_model(prediction_model=prediction_model, model=model, detector=detector,
                                           nms=args.nms, class_specific_filter=args.class_specific_filter,
                                           anchor_params=anchor_parameters)  # memory leak here
...
def Convert(prediction_model, model, detector=None, args=[]):
    # parse arguments
    args = parse_args(args)
    # check if this is indeed a training model
    anchor_parameters = None
    ...
    model, detector = models.convert_model(prediction_model=prediction_model, model=model, detector=detector,
                                           nms=args.nms, class_specific_filter=args.class_specific_filter,
                                           anchor_params=anchor_parameters, nms_threshold=args.nms_threshold)  # Memory leak here
    return model
